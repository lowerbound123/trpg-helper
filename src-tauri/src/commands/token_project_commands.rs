...
pub fn create_token_project(
    ...
    if let Some(object) = document.as_object_mut() {
        ...
        object.insert("updatedAt".to_string(), Value::String(Utc::now().to_rfc3339()));
    }
    ...
    ensure_token_folder(&app, metadata.get("folder").and_then(Value::as_str).unwrap_or_default())
        .map_err(String::from)?;
...
pub fn list_token_projects(app: AppHandle) -> CommandResult<Vec<TokenProjectSummary>> {
    ...
    for entry in fs::read_dir(root).map_err(AppError::from).map_err(String::from)? {
        ...
        if !path.is_dir() || !path.join("token.json").exists() { continue; }
...
#[tauri::command]
pub fn open_token_project(app: AppHandle, project_id: String) -> CommandResult<TokenProjectPayload> {
...
pub fn save_token_project(
    ...
    let folder = existing.metadata.get("folder").and_then(Value::as_str).unwrap_or_default();
    let metadata = serde_json::json!({
        "id": project_id,
        "folder": folder,
        "savedAt": Utc::now().to_rfc3339(),
...
pub fn rename_token_project(
    ...
    if let Some(object) = payload.metadata.as_object_mut() {
        object.insert("savedAt".to_string(), Value::String(Utc::now().to_rfc3339()));
    }
...
#[tauri::command]
pub fn copy_token_project(app: AppHandle, project_id: String) -> CommandResult<TokenProjectPayload> {
    ...
    if let Some(object) = payload.document.as_object_mut() {
        object.insert("id".to_string(), Value::String(id.clone()));
        let title = object.get("title").and_then(Value::as_str).unwrap_or("Untitled token project");
...
pub fn save_token_project_preview(
    ...
    let path = token_project_dir(&app, &project_id).map_err(String::from)?.join("preview.webp");
    fs::write(&path, decode_data_url(&data_url).map_err(String::from)?)
        .map_err(AppError::from).map_err(String::from)?;
...
pub fn rename_token_project_folder(
    ...
    index.folders = index.folders.iter().map(|folder| rename_folder_value(folder, &old_folder, &new_folder)).collect();
    index.folders.sort(); index.folders.dedup();
    ...
    for project in list_token_projects(app.clone())? {
        ...
        if next != project.folder { move_token_project(app.clone(), project.id, next)?; }
    }
    Ok(index.folders)
}

#[tauri::command]
pub fn delete_token_project_entries(app: AppHandle, entries: DeleteEntries) -> CommandResult<Vec<String>> {
    let ids: HashSet<String> = entries.ids.into_iter().collect();
    let folders: Vec<String> = entries.folders.into_iter().map(normalize_folder).filter(|value| !value.is_empty()).collect();
    for project in list_token_projects(app.clone())? {
        if ids.contains(&project.id) || folders.iter().any(|folder| folder_is_or_descendant(&project.folder, folder)) {
            remove_dir_if_exists(token_project_dir(&app, &project.id).map_err(String::from)?).map_err(String::from)?;
        }
    }
    let mut index = read_token_folders(&app).map_err(String::from)?;
    index.folders.retain(|value| !folders.iter().any(|folder| folder_is_or_descendant(value, folder)));
    write_token_folders(&app, &index).map_err(String::from)?;
    Ok(index.folders)
}
